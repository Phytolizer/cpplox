...
const std::unordered_map<std::string_view, lox::token_type>
    lox::scanner::keywords {
...
{
  if (static_cast<std::string_view::size_type>(m_current) + 1
      >= m_source.size()) {
    return '\0';
  }
...
void lox::scanner::string()
{
  ...
  auto value =
      std::string {&m_source[static_cast<std::string::size_type>(m_start) + 1],
...
void lox::scanner::identifier()
{
  ...
  auto value = std::string_view {
      &m_source[m_start],
      static_cast<std::string_view::size_type>(m_current - m_start)};
